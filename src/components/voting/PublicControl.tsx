...
import { updatePublicScore } from '../../utils/scoringEngine';
import { Smartphone, Eye, Radio, CheckCircle, ArrowRight, Minus, Plus, XCircle } from 'lucide-react';
...
export const PublicControl: React.FC<PublicControlProps> = ({
  show,
  onUpdateShow,
  onPreviewGraphic,
  onTakeGraphic,
  ...
  // STEP 1: Point Value (Default 20 pts, blocks of 4)
  const [selectedPoints, setSelectedPoints] = useState<number>(20);
  ...
  const buildVoteLayer = (): GraphicLayer => {
    ...
    return {
      id: `layer-vote-public-${timestamp}`,
      type: 'vote_award',
      name: `Televoto Público: +${selectedPoints} PTS → ${selectedParticipant?.name || selectedParticipant?.artist}`,
      category: 'VOTING',
      isOnAir: false,
      inPreview: true,
      zIndex: 25,
  ...
  const handleConfirmScore = () => {
    ...
    const updated = updatePublicScore(show, selectedRecipientId, selectedPoints);
    updated.votingStage = 'public';
    updated.activeVotingPhase = 'public';
    updated.activeReveal = {
      participantId: selectedRecipientId,
      phase: 'public',
      type: 'public',
      revealType: 'publicReveal',
      pointsAwarded: selectedPoints,
    ...
    <div className="flex flex-col gap-4 p-4 bg-[#090d16] rounded-md border border-slate-800 text-slate-100 select-none shadow-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded bg-rose-950/80 border border-rose-500/40 flex items-center justify-center text-rose-400">
            <Smartphone className="w-3.5 h-3.5" />
          </div>
          <div>
            <h2 className="font-broadcast text-xs font-bold tracking-wide uppercase text-white">
              Televoto Público — Control Operativo
            </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono font-bold text-rose-400 bg-rose-950/60 border border-rose-800/60 px-2 py-0.5 rounded">
            FASE 3: TELEVOTO PÚBLICO
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-3">
        ...
        <div className="col-span-6 flex flex-col gap-2 bg-slate-900/60 p-3 rounded border border-slate-800">
          <div className="flex items-center justify-between">
            <span className="font-broadcast text-xs font-bold uppercase tracking-wider text-rose-300 flex items-center gap-1.5">
              <span className="w-4 h-4 rounded-full bg-rose-500 text-white flex items-center justify-center text-[9px] font-black">
                1
              </span>
              PUNTOS ASIGNADOS
            </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2.5 py-1.5 rounded bg-slate-800 hover:bg-slate-700 text-rose-400 font-mono font-bold text-xs border border-slate-700 active:scale-95"
            >
              <Minus className="w-3 h-3" />
            </button>

            <div className="flex items-center bg-black/80 border border-rose-400/80 rounded px-3 py-1">
              <input
                type="number"
                step="4"
                min="0"
                ...
                className="w-12 bg-transparent text-center font-heavy font-mono-num text-lg font-black text-rose-300 outline-none"
              />
        ...
        <div className="col-span-6 flex flex-col gap-2 bg-slate-900/60 p-3 rounded border border-slate-800">
          ...
          >
            {competitionParticipants.map((p) => {
              const currentScore = scores[p.id]?.publicScore ?? 0;
              return (
                <option key={p.id} value={p.id}>
                  #{p.performanceNumber} - {p.name || p.artist} (Público: {currentScore} pts | Total: {scores[p.id]?.totalScore ?? 0} pts)
                </option>
              );
            })}
          </select>
        ...
        <div className="flex items-center gap-3">
          <div className="flex flex-col">
            ...
            <span className="font-broadcast text-xs font-black text-white uppercase">
              <span className="text-rose-400 font-mono-num font-bold">+{selectedPoints} PTS</span> →{' '}
